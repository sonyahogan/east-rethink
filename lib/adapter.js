...
Adapter.prototype.markExecuted = function (name, callback) {
    ...
    this.db[tableName].insert({ name: name, created: Date.now() })
        .then((id) => callback(null), callback);
};
...
Adapter.prototype.unmarkExecuted = function (name, callback) {
    ...
    this.db[tableName].remove({ name: name })
